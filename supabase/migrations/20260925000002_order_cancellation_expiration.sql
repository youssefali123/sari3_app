-- Migration B (feature 005-order-cancellation-expiration):
-- customer self-service cancellation, 30-minute pending expiration,
-- actor-aware notifications, and history soft-hiding.
--
-- Requires 20260925000001_add_expired_order_status.sql (enum value).

-- ============================================================================
-- 1. orders.customer_hidden_at (soft-hide, never delete — Principle VII)
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_orders_customer_hidden
  ON public.orders(customer_id, customer_hidden_at) WHERE customer_hidden_at IS NULL;

-- ============================================================================
-- 2. notification_events: two new event types
-- ============================================================================
...
-- ============================================================================
-- 3. validate_order_transition — reopen customer cancellation (all active
--    states), permit pending -> expired. Terminal states stay default-denied.
--    Safe because the ONLY writer of X -> cancelled is cancel_order()
--    (ownership-checked SECURITY DEFINER); clients hold no UPDATE policies.
-- ============================================================================
  ...
  IF NEW.status IS DISTINCT FROM OLD.status THEN
    IF NOT (
      ...
      RAISE EXCEPTION 'Invalid order status transition: % -> %', OLD.status, NEW.status
        USING ERRCODE = '23514',
              HINT = 'Valid transitions are defined in OrderStatus.ts and must match this trigger. Cancelled is reachable from pending, accepted, preparing, out_for_delivery via customer cancel (cancel_order RPC). Expired is reachable only from pending (expiration job).';
    END IF;
  END IF;
...
-- ============================================================================
-- 4. pending_order_ttl — single source of truth for the 30-minute threshold
-- ============================================================================
...
-- ============================================================================
-- 5. claim_order — add atomic 30-minute age guard (FR-011). Preserves the
--    v_claimed capture fix from 20260919000012 (PERFORM clobbers FOUND).
-- ============================================================================
...
BEGIN
  ...
  -- Atomic claim with the live 30-minute TTL guard: a stale pending order can
  -- never be claimed even if the scheduled expiration job has not run yet.
...
-- ============================================================================
-- 6. get_available_orders — exclude stale orders past the TTL.
--    Keeps the live 'storeNeighbourhood' projection key (I4 rename).
-- ============================================================================
...
-- ============================================================================
-- 7. emit_driver_pool_signal — treat 'expired' as a pool exit like the
--    other terminal leave-states (alongside, not replacing, 'rejected').
-- ============================================================================
...
-- ============================================================================
-- 8. enqueue_order_notification — actor-aware cancellation (suppress
--    customer self-echo; alert the assigned driver) + order_expired events.
-- ============================================================================
...
BEGIN
  ...
  ELSIF NEW.status = 'cancelled' AND OLD.status IS DISTINCT FROM 'cancelled' THEN
    IF v_actor = 'customer' THEN
      -- Customer-initiated: no customer self-echo; alert the assigned driver only.
      ...
      RETURN NEW;
    ELSE
      -- Non-customer actor (reserved for future store/admin cancellation flows)
...
-- ============================================================================
-- 9. cancel_order — customer self-service cancellation (single write path)
-- ============================================================================
...
BEGIN
  ...
  -- Identify the actor for the notification trigger (transaction-scoped)
  ...
  -- Release the assigned driver's capacity immediately
...
-- ============================================================================
-- 10. expire_stale_orders — scheduled batch expiration (pg_cron worker)
-- ============================================================================
  ...
  WITH expired_rows AS (
    UPDATE public.orders
     ...
     WHERE status = 'pending'
       AND driver_id IS NULL
       AND created_at < (now() - pending_order_ttl())
    RETURNING id
  )
...
-- ============================================================================
-- 11. hide_order — customer history soft-hiding (never deletes)
-- ============================================================================
...
-- ============================================================================
-- 12. advance_order_status — guarded WHERE prevents unhandled trigger
--     exceptions when a customer cancels concurrently (US7)
-- ============================================================================
...
BEGIN
  ...
  -- Guarded conditional update: a concurrent customer cancellation yields a
  -- handled response instead of a raised trigger exception (US7).
  UPDATE public.orders
...
-- ============================================================================
-- 13. get_driver_history — add customer-cancelled orders that were assigned
--     to the caller (superseding declined/released entries per 004 semantics)
-- ============================================================================
...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(h ORDER BY h.order_date DESC), '[]'::jsonb)
    ...
    FROM (
      ...
      -- 2. Cancelled while assigned to the caller (customer cancellation)
      SELECT o.id AS id,
             o.id AS order_id,
             o.restaurant_name AS store_name,
      ...
      -- 3. Declined orders (superseded by 'cancelled' once cancelled)
      SELECT doi.id AS id,
             o.id AS order_id,
             o.restaurant_name AS store_name,
      ...
      -- 4. Released orders (superseded by 'cancelled' once cancelled)
      SELECT doi.id AS id,
             o.id AS order_id,
             o.restaurant_name AS store_name,
             doi.created_at AS order_date,
      ...
      -- 5. Cancelled orders the caller interacted with (declined/released)
      --    but was not assigned to at cancellation time — one entry per order.
...
-- ============================================================================
-- 14. Drop the legacy direct client cancel policy — cancel_order() is now
--     the single authoritative write path into 'cancelled'.
-- ============================================================================
DROP POLICY IF EXISTS orders_update_own_customer_cancel ON public.orders;

-- ============================================================================
-- 15. pg_cron schedule: expire stale pending orders every minute
-- ============================================================================
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    -- Unschedule only if a previous schedule exists (unschedule errors otherwise)
    IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'expire_stale_orders_job') THEN
