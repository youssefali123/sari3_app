-- Atomic order placement RPC: validates store open status, fetches canonical
-- prices, validates/redeems a single coupon, and inserts immutable order/item/
-- address snapshots in one transaction (Constitution Principles V, VI, VII).
...
BEGIN
  ...
  -- 3. Delivery address validation & immutable snapshot
  ...
  -- 4. Canonical price validation for items and add-ons
  ...
  -- 5. Coupon validation & redemption (single coupon, row-locked)
  ...
  -- 6. Totals (delivery fee fixed at 0 in the foundation phase)
  v_final_total := v_subtotal - v_discount + v_delivery_fee;

  -- 7. Immutable snapshot persistence
  ...
  -- 8. Return serialized order (camelCase, matching the Order entity)
